...
for single_date in daterange(date(2018,5,7), date(2019,2,1)):
    ...
    for route in routes:
        ...
        for attempt in range(10):
            try:
                r = requests.get('https://rtl2.ods-live.co.uk/api/trackingHistory', params={'service':route, 'date':single_date.strftime("%Y-%m-%d"), **payload_key}, timeout=100)
        ...
        for attempt in range(10):
            try:
                r = requests.get('https://rtl2.ods-live.co.uk/api/scheduledJourneys', params={'service':route, 'date':single_date.strftime("%Y-%m-%d"), **payload_key}, timeout=100)
