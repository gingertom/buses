...
for single_date in daterange(date(2017, 1, 14), date(2017, 1, 16)):
    ...
    for file in files:
        ...
        for stop in tracking_data:
            if ("VehicleCode" in stop is False) or stop["VehicleCode"] == "":
                continue
    ...
    for bus_number, bus_dict in vehicles.items():
        ...
        bus_dict["departureTimes"] = np.array(
            bus_dict["departureTimes"], dtype="datetime64"
        )
        bus_dict["arrivalTimes"] = np.array(
            bus_dict["arrivalTimes"], dtype="datetime64"
        )

        # If this bus does more than one line this day
        if np.all(bus_dict["lines"] == bus_dict["lines"][0]) is True:

            vehicles_to_save[bus_number] = [
                {
                    "line": bus_dict["lines"][0],
                    "start": str(bus_dict["arrivalTimes"][0]),
                    "end": str(bus_dict["departureTimes"][-1]),
                }
            ]
            ...
            lines_shifted = np.concatenate(
                ([bus_dict["lines"][0]], bus_dict["lines"][:-1])
            )
            ...
            for run in zip(starts, ends):
                ...
                vehicles_to_save[bus_number].append(
                    {
                        "line": bus_dict["lines"][run[0]],
                        "start": str(bus_dict["arrivalTimes"][run[0]]),
                        "end": str(bus_dict["departureTimes"][run[1]]),
                    }
                )

    s3.uploadJsonObject(
        vehicles_to_save, f"bussesAndRoutes/{single_date.strftime('%Y-%m-%d')}.json"
    )
