...
for single_date in daterange(date(2017, 5, 25), date(2019, 2, 1)):
    ...
    for bus in busses:
        ...
        for attempt in range(10):
            try:
                r = requests.get(
                    "https://rtl2.ods-live.co.uk/api/vehiclePositionHistory",
                    params={
                        "vehicle": bus,
                ...
                if (
                    "status" in location_data
                    and location_data["status"] == "false"
                    and "message" in location_data
                    and location_data["message"] == "No data Found"
                ):
                    printed_x = True
                    print("x", end="")
                ...
                if (
                    ...
                    and location_data["status"] == "false"
                    and "message" in location_data
                    and location_data["message"].startswith("Too much data")
                ):
                    r1 = requests.get(
                        "https://rtl2.ods-live.co.uk/api/vehiclePositionHistory",
                        params={
                    ...
                    s3.uploadJsonObject(
                        r1.json(),
                        f"vehiclePositionHistory/{single_date.strftime('%Y-%m-%d')}/vehicle-AM-{bus}.json",
                    )
                    s3.uploadJsonObject(
                        r2.json(),
                        f"vehiclePositionHistory/{single_date.strftime('%Y-%m-%d')}/vehicle-PM-{bus}.json",
                    )

                s3.uploadJsonObject(
                    r.json(),
                    f"vehiclePositionHistory/{single_date.strftime('%Y-%m-%d')}/vehicle-{bus}.json",
                )
