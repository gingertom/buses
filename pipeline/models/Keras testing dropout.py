...
def create_fully_connected(input_width, dropout):
    model = Sequential()
...
def create_LSTM(input_shape, dropout, recurant_dropout):
...
def full_connected(
    ...
    model = create_fully_connected(train_matrix.shape[1], dropout)
...
def lstm(
    ...
    model = create_LSTM(
        (train_matrix_st.shape[1], train_matrix_st.shape[2]), dropout, recurant_dropout
    )
...
if __name__ == "__main__":
    ...
    with open(f"dropout-results-{days}.csv", "w") as file:

        file.write("method,dropout,recurant_dropout,loss_weight,MAPE\n")
        file.write(f"medians,nan,nan,nan,{MAPE(test_medians, test_durations)}\n")
        file.flush()

        for dropout in np.linspace(0, 0.5, 5):
            for recurant_dropout in np.linspace(0, 0.5, 5):
                ...
                file.write(f"NN,{dropout},nan,nan,{MAPE(predict_NN, test_durations)}\n")
                ...
                file.write(
                    f"LSTM,{dropout},{recurant_dropout},nan,{MAPE(predict_NN_lstm, test_durations)}\n"
                )
                file.flush()

                for intermediate_weight in np.linspace(0, 0.8, 4):
                    ...
                    file.write(
                        f"Combined,{dropout},{recurant_dropout},{intermediate_weight},{MAPE(predict_NN_combined, test_durations)}\n"
                    )
                file.flush()
